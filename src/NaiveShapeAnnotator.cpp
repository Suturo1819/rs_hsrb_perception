/**
 * This is basically the PrimitiveShapeAnnotator from the RoboSherlock core package
 * with a few adjustments
 *
 * Copyright 2014-2018 University of Bremen, Institute for Artificial Intelligence
 * Author(s): Fenja Kollasch, Ferenc Balint-Benczedi <balintbe@cs.uni-bremen.de>
 *
...
class NaiveShapeAnnotator : public DrawingAnnotator
{
...
public:
    NaiveShapeAnnotator(): DrawingAnnotator(__func__)
    {
    }
    ...
    TyErrorId processWithLock(CAS &tcas, ResultSpecification const &res_spec)
    {
      ...
      for(auto cluster : clusters)
      {
        ...
        proj.setInputCloud(cluster_cloud); // boundary cloud
        ...
        outInfo("Projected Cloud has " << cluster_projected->points.size() << " data point");
        outInfo("Number of CIRCLE inliers found " << circle_inliers->indices.size());
        outInfo("Number of line inliers found " << line_inliers->indices.size());
        ...
        else if((float)circle_inliers->indices.size() / cluster_projected->points.size() > 0.3)
        {
          ...
          *dispCloudPtr_ += *cluster_projected;
          ...
          // This is a very noob-like attempt to detect a cylinder...
        else if((float)circle_inliers->indices.size() / cluster_projected->points.size() > 0.15 &&
                (float)line_inliers->indices.size() / cluster_projected->points.size() > 0.15 ) {
          ...
          *dispCloudPtr_ += *cluster_projected;
          shapeAnnot.shape.set("cylinder");
          cv::putText(rgb_, "cylinder", cv::Point(rect.x, rect.y - 10), cv::FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(255, 255, 255), 2);
          shapeAnnot.confidence.set((float)circle_inliers->indices.size() / cluster_projected->points.size());
          cluster.annotations.append(shapeAnnot);
        }
